...
export default async function handler(req, res) {
  ...
  const { action, query, useTags, message, assistantId, tagIndex, byIndex, link } = req.query;
  ...
  try {
    switch (action) {
      ...
      case "chat":
        if (!message) {
          return res.status(400).json({ error: "Message parameter is required for chat" });
        }
      ...
      case "getId":
        if (!link) {
          return res.status(400).json({ error: "Link parameter is required for getId" });
        }
        const idResult = await neko.getId(link);
        return res.status(200).json({ id: idResult });
