...
export default async function handler(req, res) {
  ...
  const { action, number, key, otp, token, numbers } = req.query;

  try {
    switch (action) {
      case "login":
        if (!number) {
          return res.status(400).json({ error: "Number parameter is required for login" });
        }
      ...
      case "otp":
        if (!number || !key || !otp) {
          return res.status(400).json({ error: "Number, key, and otp parameters are required for OTP verification" });
        }
      ...
      case "search":
        if (!token || !number) {
          return res.status(400).json({ error: "Token and number parameters are required for search" });
        }
      ...
      case "search_bulk":
        if (!token || !numbers) {
          return res.status(400).json({ error: "Token and numbers parameter are required for bulk search" });
        }
