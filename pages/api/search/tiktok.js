...
const randomString = (
  ...
  formatNumber = (num) => {
    ...
    return numDigits <= 3
      ? numString
      : `${(num / 1e3 ** Math.floor((numDigits - 1) / 3)).toFixed(1).replace(/\.0$/, "")}${["", "k", "M", "B", "T"][Math.floor((numDigits - 1) / 3)]}`;
  };
...
function formatDuration(seconds) {
  const h = Math.floor(seconds / 3600),
    ...
    s = seconds % 60;
  return [h && `${h} hour`, m && `${m} minute`, s && `${s} second`]
    .filter(Boolean)
    .join(" ");
}
