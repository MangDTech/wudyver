  ...
  try {
    ...
    if (quotes.length > 0) {
      const filteredQuotes = quotes
        .split("\n")
        .filter((_, index) => index !== 0 && index !== quotes.length - 1); // Menghapus baris pertama dan terakhir

      const randomQuote = filteredQuotes[
        Math.floor(Math.random() * filteredQuotes.length)
      ].slice(1, -2); // Mengambil quote secara acak dan membersihkan karakter yang tidak diinginkan
