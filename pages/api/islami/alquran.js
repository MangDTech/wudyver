...
export default async function handler(req, res) {
  const { ayah, surah, edition = "quran-simple" } = req.query;

  if (!ayah && !surah) return res.status(400).json({ error: "Parameter 'ayah' atau 'surah' wajib diisi." });
  ...
  try {
    if (ayah) {
      ...
      res.status(200).json({ ...surahData, additionalData });
    }
  } catch (error) {
