...
async function fetchImageUrl(url) {
  try {
    ...
    return "https:" + $("tr.mergedtoprow td.infobox-full-data.maptable div.ib-settlement-cols-row div.ib-settlement-cols-cell a.mw-file-description img.mw-file-element")
      .attr("src") || null;
  } catch (error) {
    return null;
  }
...
export default async function handler(req, res) {
  try {
    ...
    if (kabupatenList.length === 0) return res.status(404).json({ error: 'No kabupaten found' });
    ...
    console.error(error);
    res.status(500).json({ error: 'Failed to fetch kabupaten data' });
  }
}
