...
export default async function handler(req, res) {
  ...
  if (!name || !type) {
    return res.status(400).json({ error: 'Username dan jenis provider wajib diisi.' });
  }
  ...
  try {
    switch (type.toLowerCase()) {
      ...
      case 'threads':
        ...
        break;
      default:
        return res.status(400).json({ error: 'Jenis provider tidak valid. Pilih antara Instagram, Tiktok, atau Threads.' });
    }
