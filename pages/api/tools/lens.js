...
export default async function handler(req, res) {
  if (req.method === 'GET') {
    ...
    try {
      const imageData = Buffer.from(await (await fetch(url)).arrayBuffer());
      ...
      const uploadResponse = await fetch(
        `https://lens.google.com/v3/upload?s=4&re=df&stcs=${url}`,
        {
          method: "POST",
          headers: {
            "User-Agent":
      ...
      return res.status(500).json({ error: 'Script element not found' });
    } catch (error) {
      return res.status(500).json({ error: `Error fetching or processing data: ${error.message}` });
    }
  }
