...
class smstome {
  ...
  async getNumber(country) {
    try {
      const { data } = await axios.get(
        `https://smstome.com/country/${country.toLowerCase()}`
      );
      const $ = cheerio.load(data);
      return $(".numview")
        .map((_, numview) => ({
          phoneNumber: $("a", numview).text().trim(),
          location: $("div.row:nth-child(1) > div > small", numview)
            .text()
            .trim(),
  ...
  async getMessage(url, page) {
    try {
      ...
      const $ = cheerio.load(data);
      return $("table.messagesTable tbody tr")
        .map((_, message) => ({
          from: $("td:nth-child(1)", message)
            .text()
            .trim()
            .replace("\x3c!--sse--\x3e", "")
            .replace("\x3c!--/sse--\x3e", ""),
          received: $("td:nth-child(2)", message)
            .text()
            .trim()
            .replace("\x3c!--sse--\x3e", "")
            .replace("\x3c!--/sse--\x3e", ""),
          content: $("td:nth-child(3)", message)
            .text()
            .trim()
            .replace("\x3c!--sse--\x3e", "")
            .replace("\x3c!--/sse--\x3e", ""),
...
class sms24 {
    ...
    try {
      ...
      return $(".callout")
        .map((_, callout) => ({
          ...
          link:
            "https://sms24.me/en/countries/" +
            $("span.fi", callout).attr("data-flag"),
  ...
  async getNumber(country) {
    try {
      const { data } = await axios.get(
        `https://sms24.me/en/countries/${country.toLowerCase()}`
      );
      const $ = cheerio.load(data);
      return $(".callout")
        .map((_, callout) => ({
  ...
  async getMessage(number) {
    try {
      const { data } = await axios.get(
        `https://sms24.me/en/numbers/${parseInt(number)}`
      );
      const $ = cheerio.load(data);
      return $(".shadow-sm.bg-light.rounded.border-start.border-info.border-5")
        .map((_, message) => ({
...
export default async function handler(req, res) {
  ...
  switch (method) {
    case "GET":
      const { service, action, country, number, page } = req.query;
