...
export default async function handler(req, res) {
  ...
  try {
    ...
    const result = 
      source === "random" ? await wall.random({ resolution }) :
      source === "wallhaven" ? await wall.search({ title: query, page, type, aiArt: aiArt === "true" }, AnimeSource.WallHaven) :
      source === "wallpapers" ? await wall.search({ title: query }, AnimeSource.Wallpapers) :
      source === "zerochan" ? await wall.search({ title: query }, AnimeSource.ZeroChan) :
      source === "live2d" ? await wall.live2d(live2d) :
      source === "hoyolab" ? await wall.hoyolab({ game, postType }) :
      source === "pinterest" ? await wall.pinterest(query) :
      null;

    if (!result) return res.status(400).json({ success: false, error: "Invalid source parameter." });
