...
const baseCerpen = "http://cerpenmu.com/100-cerpen-kiriman-terbaru",
  ...
  baseOtakudesu = "https://otakudesu.wiki/",
  baseFilmApik = "https://film-apik.online/",
  baseSSS = "https://instasupersave.com/",
  convertMs = (duration) => {
...
async function findSongs(text) {
  try {
    const { data } = await axios.get(
      ...
      { data: lyricData } = await axios.get(
        `https://songsear.ch/api/song/${result.number}?text_only=true`,
      ),
      lyrics = lyricData.song.text_html
        .replace(/<br\/>/g, "\n")
        .replace(/&#x27;/g, "'");
    return {
      status: !0,
...
async function igStalk(username) {
  try {
    const { data } = await axios.get(
      ...
      pronouns =
        0 === data.result.user.pronouns.length
          ? ""
          : data.result.user.pronouns.join("/"),
