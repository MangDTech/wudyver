...
async function fetchStickerSet(query) {
  const response = await fetch(
    `https://api.telegram.org/bot${BOT_TOKEN}/getStickerSet?name=${encodeURIComponent(query)}`
  );
...
export default async function handler(req, res) {
  const { query, all, sticker } = req.query;
  ...
  if (all === "true") {
    const result = await Promise.all(
      stickers.map(async (sticker) => {
        ...
        return { ...sticker, fileUrl };
      })
    );
    return res.status(200).json({ stickers: result });
  }
  ...
  if (sticker) {
    ...
    if (index >= 0 && index < stickers.length) {
      ...
      return res.status(200).json({ sticker: { ...stickers[index], fileUrl } });
    }
    return res.status(404).json({ error: "Stiker tidak ditemukan." });
  }

  return res.status(400).json({ error: "Parameter 'all' atau 'sticker' diperlukan." });
}
